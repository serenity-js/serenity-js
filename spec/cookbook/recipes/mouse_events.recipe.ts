...
class MouseEvents {
    ...
    static Triggerred_Result = Target.the('event-triggering button').located(by.css('#event-tester-{0} pre'));
}
...
synced.describe ('When demonstrating the usage of a Static Website, a test scenario', function () {
    ...
    synced.before(() => james.attemptsTo(
        Open.browserOn(app.demonstrating('mouse_events'))
    ));
    ...
    synced.it ('can click the on-screen elements', () =>
        james.attemptsTo(
            ...
            expect(james.toSee(Text.of(MouseEvents.Triggerred_Result.of('click'))))
    ...
    synced.it ('can double-click the on-screen elements', () =>
        james.attemptsTo(
            ...
            expect(james.toSee(Text.of(MouseEvents.Triggerred_Result.of('dblclick'))))
