...
Since [`PageElements`](/api/web/class/PageElements) class produces a `List<PageElement>`, it also offers this functionality.
...
[`PageElements`](/api/web/class/PageElements) produces a `List<PageElement>`, so it also offers a [`forEach`](/api/web/class/PageElements#forEach) method:
