...
to a more modern integration tool like [Protractor](/handbook/test-runners/webdriverio)
or [Playwright](/handbook/test-runners/playwright) in the next step.
...
- use `protractor.conf.js` to [configure Serenity/JS](/api/protractor/interface/ProtractorConfig) to use the reporting services you want to use,
...
by providing a custom implementation via [`serenity.actors`](/api/protractor/interface/ProtractorConfig#serenity)
