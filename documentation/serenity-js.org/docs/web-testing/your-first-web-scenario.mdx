...
- Serenity/JS test scenarios are **actor-centric** and follow the [**Screenplay Pattern**](/handbook/design/screenplay-pattern)
- Serenity/JS uses an activity-based code reuse model
...
:::info Pro Tip
If you get lost or stumble upon a problem you're not quite sure how to solve - **[Serenity/JS Community Chat](https://matrix.to/#/#serenity-js:gitter.im)** is here to help.
:::
...
:::info Remember
**Actors** perform **activities** to interact with the system under test.
:::
...
Serenity/JS activity-based programming model applies to performing assertions just as well as it applies
...
All [Serenity/JS assertions](/api/assertions) follow the **same consistent pattern**
...
Serenity/JS also ships with [web-specific expectations](/api/web#Index) like [`isVisible()`](/api/web/function/isVisible)
...
- Automatically taking screenshots on failure
- Tracing
- Videos
- Playwright reports
...
Page elements are questions, questions are the way for actors to retrieve information about their environment and the state of the system under test

## Running tests on CI

## Viewing reports

I've configured all the [Serenity/JS project templates](/handbook/getting-started#serenityjs-project-templates)
to produce
