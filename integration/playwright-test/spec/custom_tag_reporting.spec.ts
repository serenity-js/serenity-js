...
import { SceneFinished, SceneFinishes, SceneStarts, SceneTagged, TestRunFinished, TestRunFinishes, TestRunnerDetected, TestRunStarts } from '@serenity-js/core/events';
import { ArbitraryTag, CapabilityTag, CorrelationId, ExecutionSkipped, ExecutionSuccessful, FeatureTag, IssueTag, ManualTag, Name } from '@serenity-js/core/model';
...
describe('@serenity-js/playwright-test', function () {
    ...
    it('emits custom tags if defined via tag API on describe or test title so that Serenity BDD can aggregate them correctly', () => playwrightTest('--project=default', 'custom_tags/custom_tags_via_tag_api.spec.ts')
        .then(ifExitCodeIsOtherThan(0, logOutput))
        .then(result => {
            expect(result.exitCode).to.equal(0);
