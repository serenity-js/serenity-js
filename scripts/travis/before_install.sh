...
if [[ $TRAVIS_BRANCH == 'master' ]]; then
  ...
  git init
  git clean -dfx
  git remote add origin https://github.com/serenity-js/serenity-js.git
  git fetch origin
  git clone https://github.com/$TRAVIS_REPO_SLUG.git $TRAVIS_REPO_SLUG
  git checkout $TRAVIS_BRANCH

  git config credential.helper store
  echo "https://${RELEASE_GH_USERNAME}:${RELEASE_GH_TOKEN}@github.com/serenity-js/serenity-js.git" > ~/.git-credentials
