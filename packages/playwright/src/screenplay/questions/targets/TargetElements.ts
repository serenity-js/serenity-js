...
export class TargetElements
    extends Question<Promise<ElementHandleAnswer[]>>
    implements
    MetaQuestion<Answerable<ElementHandle>, Promise<ElementHandleAnswer[]>>
{
   ...
   */
    public static at(selector: string): TargetElements {
   ...
   */
    as(description: string): TargetElements {
    ...
    of(
        parent: Answerable<ElementHandle<Node>>
    ): Question<Promise<ElementHandleAnswer[]>> {
        return new TargetElements(this.selector, parent).as(
   ...
   */
    async answeredBy(
        ...
        const handles = await parent.$$(this.selector);

        const answers = handles.map(extend);

        return override(answers, 'toString', this.toString.bind(this));
    }

    private async getRealParent(
        actor: AnswersQuestions & UsesAbilities & PerformsActivities
