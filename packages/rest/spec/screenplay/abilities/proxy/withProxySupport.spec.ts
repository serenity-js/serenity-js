import { expect } from '@integration/testing-tools';
import axios from 'axios';
import { describe, it } from 'mocha';

import { withProxySupport } from '../../../../src/screenplay/abilities/proxy';
import { ProxyAgent } from '../../../../src/screenplay/abilities/proxy/ProxyAgent';

describe('withProxySupport', () => {

    describe('when no proxy overrides are defined', () => {

        it('overrides Axios built-in proxy mechanism to use env variables', () => {

            const instance = withProxySupport(axios.create({}));

            expect(instance.defaults.proxy).to.equal(false);
            expect(instance.defaults.httpAgent).to.be.instanceof(ProxyAgent);
            expect(instance.defaults.httpsAgent).to.be.instanceof(ProxyAgent);
            expect(instance.defaults.adapter).to.equal('http');
        });
    });

    describe('when proxy overrides are defined', () => {

        it('creates an Axios instance that overrides Axios built-in proxy mechanism', () => {

            const instance = withProxySupport(axios.create({
                proxy: {
                    protocol: 'http',
                    host: 'proxy.mycompany.com',
                    port: 9000,
                }
            }));

            expect(instance.defaults.proxy).to.equal(false);
            expect(instance.defaults.httpAgent).to.be.instanceof(ProxyAgent);
            expect(instance.defaults.httpsAgent).to.be.instanceof(ProxyAgent);
            expect(instance.defaults.adapter).to.equal('http');
        });
    });
});