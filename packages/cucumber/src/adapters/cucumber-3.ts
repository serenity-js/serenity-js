...
export = function({ notifier, mapper, cache, cucumber }: Dependencies) {

    cucumber.defineSupportCode(({ Before, After, AfterAll }) => {
        After(function() {
    ...
    return class SerenityNotifier {
        ...
        private outcomeFrom(result: { duration: number, exception: string | Error, status: string }): Outcome {
        ...
        private errorFrom(exception: Error | string) {
