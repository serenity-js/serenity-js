...
export class Timestamp extends TinyType {
    static fromJSON = (v: string) => new Timestamp(new Date(ensure(Timestamp.name, v, isSerialisedISO8601Date())));
    static fromMillisecondTimestamp = (v: number) => new Timestamp(moment(v).toDate());
