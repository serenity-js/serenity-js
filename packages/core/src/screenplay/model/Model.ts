...
export type Model<OriginalType> = {
    ...
    // make the method signature asynchronous, accepting Answerables and returning a Promise
        ? (...args: { [P in keyof OriginalParameters]: Answerable<OriginalParameters[P]> }) =>  Question<Promise<Awaited<OriginalMethodResult>>> & Interaction & Model<OriginalMethodResult>
    // is it an object? wrap each field
        : Question<Promise<Awaited<OriginalType[Field]>>> & Interaction & Model<OriginalType[Field]>
}
