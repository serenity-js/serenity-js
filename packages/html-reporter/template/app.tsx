...
import { DATA, formatDuration, getBrowserTag, matchesSearch, outcomeClass, outcomeIcon, RawHtml, relativeSourcePath, scenarioUrl, showToast } from './utils';
          ...
          <div class="scenario-item" onClick=${() => onNavigate('/tests?run=' + run.timestamp)} style="cursor:pointer">
            ...
            <div class="scenario-info">
              ...
              <div class="scenario-meta">
                ...
                ${run.branch ? html`<span>•</span><span style="display:inline-flex;align-items:center;gap:3px">${(() => { const repoUrl = run.repositoryUrl ? run.repositoryUrl.replace(/\.git$/, '').replace(/^git@([^:]+):/, 'https://$1/') : ''; return html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px;flex-shrink:0"><line x1="6" y1="3" x2="6" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/></svg>${repoUrl ? html`<a href="${repoUrl}/tree/${run.branch}" target="_blank" rel="noopener" onClick=${(e) => e.stopPropagation()} style="font-size:var(--font-xs);color:inherit;text-decoration:none" onMouseOver=${(e) => e.target.style.textDecoration='underline'} onMouseOut=${(e) => e.target.style.textDecoration='none'}>${run.branch}</a>` : html`<span style="font-size:var(--font-xs)">${run.branch}</span>`}`; })()}</span>` : null}
                ${run.commit ? html`<span>•</span><span style="display:inline-flex;align-items:center;gap:3px">${(() => { const repoUrl = run.repositoryUrl ? run.repositoryUrl.replace(/\.git$/, '').replace(/^git@([^:]+):/, 'https://$1/') : ''; return html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px;flex-shrink:0"><circle cx="12" cy="12" r="4"/><line x1="1" y1="12" x2="8" y2="12"/><line x1="16" y1="12" x2="23" y2="12"/></svg>${repoUrl ? html`<a href="${repoUrl}/commit/${run.commit}" target="_blank" rel="noopener" onClick=${(e) => e.stopPropagation()} style="font-family:var(--font-mono);font-size:var(--font-xs);color:inherit;text-decoration:none" onMouseOver=${(e) => e.target.style.textDecoration='underline'} onMouseOut=${(e) => e.target.style.textDecoration='none'}>${run.commit}</a>` : html`<span style="font-family:var(--font-mono);font-size:var(--font-xs)">${run.commit}</span>`}`; })()}</span>` : null}
