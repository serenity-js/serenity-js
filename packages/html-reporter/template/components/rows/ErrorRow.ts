...
import { formatDuration, outcomeClass, outcomeIcon, relativeSourcePath, scenarioUrl } from '../../utils';
...
export function ErrorRow({ scenario: s, duplicateCount, specDirectory, onNavigate }: ErrorRowProps): ReturnType<typeof html> {
    ...
    <div class="scenario-item" onClick=${() => onNavigate(clickTarget)}
      ...
      <div class="scenario-info">
        ...
        <div style="font-size:var(--font-sm);color:var(--color-${outcomeClass(s.outcome)});margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${s.error ? s.error.message : s.outcome}${duplicateCount > 1 ? html` <span style="font-weight:600"> (×${duplicateCount})</span>` : null}</div>
