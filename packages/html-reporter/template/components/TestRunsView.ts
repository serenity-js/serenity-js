          ...
          <div class="scenario-item" onClick=${() => onNavigate('/tests?run=' + run.timestamp)}>
            ...
            <div class="scenario-info">
              ...
              <div class="scenario-meta">
                ...
                ${run.branch ? html`<span>•</span><span class="inline-flex-center">${(() => { const repoUrl = run.repositoryUrl ? run.repositoryUrl.replace(/\.git$/, '').replace(/^git@([^:]+):/, 'https://$1/') : ''; return html`${icons.branch}${repoUrl ? html`<a href="${repoUrl}/tree/${run.branch}" target="_blank" rel="noopener" onClick=${(e: Event) => e.stopPropagation()} class="text-xs" style="color:inherit;text-decoration:none" onMouseOver=${(e: Event) => (e.target as HTMLElement).style.textDecoration='underline'} onMouseOut=${(e: Event) => (e.target as HTMLElement).style.textDecoration='none'}>${run.branch}</a>` : html`<span class="text-xs">${run.branch}</span>`}`; })()}</span>` : null}
                ${run.commit ? html`<span>•</span><span class="inline-flex-center">${(() => { const repoUrl = run.repositoryUrl ? run.repositoryUrl.replace(/\.git$/, '').replace(/^git@([^:]+):/, 'https://$1/') : ''; return html`${icons.commit}${repoUrl ? html`<a href="${repoUrl}/commit/${run.commit}" target="_blank" rel="noopener" onClick=${(e: Event) => e.stopPropagation()} class="font-mono text-xs" style="color:inherit;text-decoration:none" onMouseOver=${(e: Event) => (e.target as HTMLElement).style.textDecoration='underline'} onMouseOut=${(e: Event) => (e.target as HTMLElement).style.textDecoration='none'}>${run.commit}</a>` : html`<span class="font-mono text-xs">${run.commit}</span>`}`; })()}</span>` : null}
