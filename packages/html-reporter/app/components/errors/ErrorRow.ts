...
import { ansiToHtml, formatDuration, outcomeClass, relativeSourcePath, scenarioUrl, stripAbsolutePaths } from '../../utils';
...
export function ErrorRow({ scenario: s, duplicateCount, specDirectory, onNavigate }: ErrorRowProps): ReturnType<typeof html> {
    ...
    <div class="scenario-item" onClick=${() => onNavigate(clickTarget)}
      ...
      <div class="scenario-info">
        ...
        <div style="font-size:var(--font-sm);color:var(--color-${outcomeClass(s.outcome)});margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" dangerouslySetInnerHTML=${{ __html: ansiToHtml(stripAbsolutePaths(s.error ? s.error.message : s.outcome, specDirectory)) + (duplicateCount > 1 ? ` <span style="font-weight:600"> (×${duplicateCount})</span>` : '') }}></div>
