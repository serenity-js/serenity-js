...
export function TimelineView({
    ...
    const statsContent = html`
        <div class="kpi-row grid-4col">
            <${ KpiCard } label="Slowest" value=${ formatDuration(slowest) }
                          ariaLabel="Slowest test: ${ formatDuration(slowest) }"
                          valueColor=${ slowest > avg * 3 ? 'var(--color-failed)' : slowest > avg * 2 ? 'var(--color-pending)' : '' }/>
                <${ KpiCard } label="Fastest" value=${ formatDuration(fastest) }
                              ariaLabel="Fastest test: ${ formatDuration(fastest) }"/>
                <${ KpiCard } label="Average" value=${ formatDuration(avg) }
                              ariaLabel="Average duration: ${ formatDuration(avg) }"/>
                <${ KpiCard } label="Total" value=${ formatDuration(summary.duration) }
                              ariaLabel="Total duration: ${ formatDuration(summary.duration) }"
                              subtitle="${ allScenarios.length } scenarios"/>
        </div>
    `;

    const topbarActions = html`
        <${ TopbarActions } onOpenFilter=${ sheets.openFilter } onOpenSort=${ sheets.openSort }
                            onOpenStats=${ sheets.openStats }/>`;
