...
All timestamps in the HTML report are currently displayed in UTC (from the ISO 8601 strings stored in `db.json`).
When a user in London views a report generated by a CI runner in UTC, the times look correct — but for users in
other time zones, "24 Jul 2026 21:36" doesn't match their local sense of when the build ran.

Since the report is a self-contained HTML file viewed in a browser, we have access to the user's local time zone
via `Intl.DateTimeFormat` and `Date` — we should use it.
