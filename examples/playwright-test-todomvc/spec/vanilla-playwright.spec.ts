...
test.describe('Vanilla Playwright Test', () => {
    ...
    test.describe('Mark all as completed', () => {
        ...
        test('should allow me to mark all items as completed', async ({ page }) => {
            ...
            await expect(page.locator('.todo-list li')).toHaveClass(['todo completed', 'todo completed', 'todo completed']);
        ...
        test('should allow me to clear the complete state of all items', async ({ page }) => {
            ...
            await expect(page.locator('.todo-list li')).toHaveClass(['todo', 'todo', 'todo']);
        });
    ...
    test.describe('Item', () => {
        ...
        test('should allow me to mark items as complete', async ({ page }) => {
            ...
            const firstTodo = page.locator('.todo-list li').nth(0);
            await firstTodo.locator('.toggle').check();
            await expect(firstTodo).toHaveClass('todo completed');

            // Check second item.
            const secondTodo = page.locator('.todo-list li').nth(1);
            await expect(secondTodo).not.toHaveClass('todo completed');
            await secondTodo.locator('.toggle').check();

            // Assert completed class.
            await expect(firstTodo).toHaveClass('todo completed');
            await expect(secondTodo).toHaveClass('todo completed');
        });
        ...
        test('should allow me to un-mark items as complete', async ({ page }) => {
            ...
            await firstTodo.locator('.toggle').uncheck();
            await expect(firstTodo).not.toHaveClass('todo completed');
            await expect(secondTodo).not.toHaveClass('todo completed');
    ...
    test.describe('Editing', () => {
        ...
        test('should save edits on pressing enter', async ({ page }) => {
            const todoItems = page.locator('.todo-list li');
        ...
        test('should remove the item when the remove button is clicked', async ({ page }) => {
            const todoItems = page.locator('.todo-list li');
    ...
    test.describe('Persistence', () => {
        test('should persist its data', async ({ page }) => {
            ...
            await expect(todoItems).toHaveClass(['todo completed', 'todo']);
...
async function checkNumberOfTodosInLocalStorage(page: Page, expected: number) {
    return await page.waitForFunction(e => {
        return JSON.parse(localStorage['serenity-js-todo-app']).length === e;
    }, expected);
}

async function checkNumberOfCompletedTodosInLocalStorage(page: Page, expected: number) {
    return await page.waitForFunction(e => {
        return JSON.parse(localStorage['serenity-js-todo-app']).filter((todo: any) => todo.completed).length === e;
    }, expected);
}

async function checkTodosInLocalStorage(page: Page, title: string) {
    return await page.waitForFunction(t => {
        return JSON.parse(localStorage['serenity-js-todo-app']).map((todo: any) => todo.name).includes(t);
    }, title);
}
